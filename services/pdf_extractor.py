...
class PDFExtractor:
    ...
    def __init__(self, use_cache=True):
        ...
        logger.info(f"Initialized lightweight PDF extractor with PyPDF2 (cache: {'enabled' if use_cache else 'disabled'})")
    ...
    def extract_text(self, pdf_bytes: bytes) -> str:
        ...
        try:
            ...
            if self.use_cache:
                ...
                logger.debug("Cache MISS - Extracting PDF text using PyPDF2")
            else:
                logger.debug("Cache disabled - Extracting PDF text using PyPDF2")
    ...
    def _perform_extraction(self, pdf_bytes: bytes) -> Tuple[str, int]:
        """
        Perform the actual PDF text extraction
        ...
        logger.info(f"Successfully extracted text from {page_count} PDF pages")
